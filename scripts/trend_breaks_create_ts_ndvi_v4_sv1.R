...
fpath <- paste0(dataf,"/modistsp/VI_16Days_250m_v61/NDVI") # NDVI
...
outf3 <- paste0(dataf2, "/timeseries/output_h5p/NDVI_250m/bfast/")  
...
#for (z in 1 ) {
  ...
  rastfiles <- list.files(path=fpath, pattern = "*NDVI*", full.names = TRUE)
  ...
  rastfilesNames <- list.files(path=fpath, pattern = "*NDVI*", full.names = FALSE)
